...
func _on_p1_finisher_meter_changed(new_finisher_meter: int) -> void:
	...
	var col := Color.GREEN
	var t := create_tween()
	$P1FinisherMeter.scale = Vector2(1.15, 1.15)
	t.tween_property($P1FinisherMeter, "scale", Vector2.ONE, 0.15)
...
func _on_p2_finisher_meter_changed(new_finisher_meter: int) -> void:
	...
	var col := Color.GREEN
	var t := create_tween()
	$P2FinisherMeter.scale = Vector2(1.15, 1.15)
	t.tween_property($P2FinisherMeter, "scale", Vector2.ONE, 0.15)
...
func _on_combo_changed(hits: int) -> void:
	if hits < 2:
		return

	$ComboLabel.text = str(hits) + " HITS"
	$ComboLabel.visible = true
	$ComboTimer.start()


func _ready() -> void:
	$ComboTimer.timeout.connect(func(): $ComboLabel.visible = false)
